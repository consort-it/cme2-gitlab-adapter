...
import com.consort.util.CommitMapper;
	...
	private void initContentAsStringRoute(final Service http) {
		...
		http.get(BASE_URI + "/:microservice-name/content-as-string/:filepath", (req, res) -> {
			...
			try {
				...
				logger.info(
						"The Route /api/v1/gitlab-adapter/" + microserviceName + "/content-as-string/" + pathToFile);
				...
				final String serverConfiguration = new ServerConfigurator().getServerConfigurationForFiles(gitlabHosts,
						projectId, pathToFile, true);
				final GitlabConnectionHandler connectionHandler = new GitlabConnectionHandler(serverConfiguration,
						accessTokens);
				...
				return mapper.writeValueAsString(
						new ErrorMessage("GIT-1234", "Something was wrong with the client request"));
	...
	private void initMetadataRoute(final Service http) {
		...
		http.get(BASE_URI + "/:microservice-name/metadata", (req, res) -> {
			...
			try {
				...
				final String serverConfigForCommitMetadata = new ServerConfigurator().getCommitMetadata(gitlabHosts,
						projectId, "");
				final GitlabConnectionHandler commitMetadataHandler = new GitlabConnectionHandler(
						serverConfigForCommitMetadata, accessTokens);

				List<Commit> commits = mapper.readValue(commitMetadataHandler.getJsonFromGitlab(),
						new TypeReference<List<Commit>>() {
						});
				List<CommitWithoutUnderline> commitsWithoutUnderline = new ArrayList<CommitWithoutUnderline>();

				for (Commit commit : commits) {
					commitsWithoutUnderline.add(CommitMapper.mapCommit(commit));
				}

				if (commitsWithoutUnderline.size() < limit) {
					return mapper.writeValueAsString(commitsWithoutUnderline);
				} else {
					return mapper.writeValueAsString(commitsWithoutUnderline.subList(0, limit));
				}

			} catch (Exception e) {

				res.status(400);
				return mapper.writeValueAsString(new ErrorMessage("GIT-1234", e.getMessage()));
